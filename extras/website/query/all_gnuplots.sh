...
./gnuplots_time.sh PO_60.png "select date_format(timedate,\"%m/%d/%H\") as time,avg(players_online) from statistics where (unix_timestamp(now())-unix_timestamp(timedate))<24*3600 group by time;" "Average Players load per hour" 0 24
