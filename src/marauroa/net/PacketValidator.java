/* $Id: PacketValidator.java,v 1.1 2004/11/27 10:31:57 root777 Exp $ */
/***************************************************************************
...
public final class PacketValidator
{
  ...
  {
    marauroad.trace("PacketValidator",">");
  ...
  {
    boolean banned = false;
    marauroad.trace("PacketValidator::checkBanned",">");
    checkReload();
    if(banList!=null)
      try
      {
        for(int i=0; i<banList.length; i++)
        {
          ...
          {
            marauroad.trace("PacketValidator::checkBanned","D","Address "+ address+" is banned by "+iam);
            banned=true;
            break;
          }
        }
  ...
  {
    marauroad.trace("PacketValidator::loadBannedIPNetworkListFromDB",">");
    try
    {
      ...
      marauroad.trace("PacketValidator::loadBannedIPNetworkListFromDB","D","loaded "+ban_list_tmp.size() + " entries from ban table");
      connection.close();
    }
    catch(Exception e)
    {
      marauroad.thrown("PacketValidator::loadBannedIPNetworkListFromDB","X",e);
    }
    finally
    {
  ...
  {
    marauroad.trace("PacketValidator::checkReload",">");
    try
    {
  ...
  {
    marauroad.trace("PacketValidator::createDBConnection",">");
    try
    {
    ...
    {
      marauroad.thrown("PacketValidator::createDBConnection","X",e);
      throw e;
    }
    finally
    {
