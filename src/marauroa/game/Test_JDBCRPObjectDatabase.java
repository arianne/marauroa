  ...
  public void testStoreRPObject()
    ...
    try
      ...
      for(int i=0;i<total;++i)
        {
        assertFalse(database.hasRPObject(new RPObject.ID(i)));
        database.storeRPObject(createObject(i));
        assertTrue(database.hasRPObject(new RPObject.ID(i)));
        }
        
      t2=new Date().getTime();
      
      JDBCRPObjectDatabase.RPObjectIterator it=database.iterator();
      ...
      for(int i=0;i<total;++i)
        {
        RPObject result=database.loadRPObject(new RPObject.ID(i));
      ...
      for(int i=0;i<total;++i)
        {
        database.deleteRPObject(new RPObject.ID(i));
        assertFalse(database.hasRPObject(new RPObject.ID(i)));
        }
