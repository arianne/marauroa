...
class TCPReader extends Thread {
	 ...
	 *            Statistics
	 */
	public TCPReader(NetworkServerManagerCallback networkServerManager,
			HashMap<InetSocketAddress, Socket> tcpSockets, Statistics stats) {
		super("TCPReader");
	...
	public void run() {
		...
		while (networkServerManager.isStillRunning()) {
			...
			Map<InetSocketAddress, Socket> temptTcpSockets = (Map<InetSocketAddress, Socket>) tcpSockets
					.clone();
			for (InetSocketAddress inetSocketAddress : temptTcpSockets.keySet()) {
				...
				try {
					...
					if (toReadInt == null) {
						...
						if (is.available() >= 4) {
							...
							if (size == 542393671) { // "GET "
								// This request was not created by the
								// marauroa-client
								// but it was created by the HTTP-client to
								// compare
								...
								os.close();
								logger
										.warn("Closing connection because packet-size is magic-number \"GET \".");
								networkServerManager
										.disconnectClient(inetSocketAddress);
								continue;
							}
					...
					if ((size > -1) && (is.available() >= size)) {
						...
						networkServerManager.receiveMessage(buffer,
								inetSocketAddress);
					...
					logger.error("error while processing tcp-packets ("
			...
			if (!found) {
				...
				try {
					...
					if (wait > 0) {
						...
						wait = wait * -1;
						logger
								.warn("Turn duration overflow by " + wait
										+ " ms");
					}
