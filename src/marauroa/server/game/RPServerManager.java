/* $Id: RPServerManager.java,v 1.37 2006/08/20 15:40:15 wikipedian Exp $ */
/***************************************************************************
...
public class RPServerManager extends Thread {
	...
	private Perception getPlayerPerception(
			PlayerEntryContainer.RuntimePlayerEntry playerEntry) {
	...
	private void sendPlayerPerception(
			PlayerEntryContainer.RuntimePlayerEntry entry,
			Perception perception, RPObject object) {
		if (perception != null) {
			...
			messages2cPerception.setPerceptionTimestamp(entry
		...
		try {
			...
			while (it_notified.hasNext()) {
				...
				try {
					...
					logger
							.error("Can't update the player(" + clientid + ")",
									e);
				}
			}
		} catch (Exception e) {
		...
		try {
			...
			while (it_notified.hasNext()) {
				...
				try {
					...
					if (id == null) {
						logger
								.debug("Can't notify a player("
										+ clientid
										+ ") that timedout because it never completed login");
					} else {
						RPObject object = getRPObject(id);

						if (ruleProcessor.onTimeout(id)) {
							/*
							 * NOTE: Set the Object so that it is stored in
							 * Database
							 */
							playerContainer.setRPObject(clientid, object);
							netMan.disconnectClient(playerContainer
									.getInetSocketAddress(clientid));
						}
					}
				} catch (Exception e) {
					logger.error("Can't notify a player(" + clientid
							+ ") that timedout");
				} finally {
			...
			logger.error(
					"Can't notify a player(-not available-) that timedout", e);
		} finally {
		...
		synchronized (contentsToTransfer) {
			for (RPObject.ID id : contentsToTransfer.keySet()) {
				try {
					...
					MessageS2CTransferREQ mes = new MessageS2CTransferREQ(
							entry.source, content);
					mes.setClientID(entry.clientid);

					netMan.sendMessage(mes);
				} catch (NoSuchClientIDException e) {
					logger.error("Unable to find client id for player: " + id,
							e);
				}
			}
	...
	public void run() {
		try {
			...
			while (keepRunning) {
				...
				try {
					logger.info("Turn time elapsed: " + ((stop - start) / 1000)
							+ " microsecs");
					...
					if (delay < 0) {
						...
						logger.warn("Turn duration overflow by " + (-delay)
								+ " ms: " + sb.toString());
					} else if (delay > turnDuration) {
						logger
								.error("Delay bigger than Turn duration. [delay: "
										+ delay
										+ "] [turnDuration:"
										+ turnDuration + "]");
						delay = 0;
