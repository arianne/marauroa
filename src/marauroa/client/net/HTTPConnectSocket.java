...
public class HTTPConnectSocket extends Socket {
	...
	private void verifyConnection() throws IOException {
		...
		if (is.read(data) < data.length) {
			throw new IOException(translate("Unexpected end of stream while reading proxy answer."));
		}
		...
		if (!answer.startsWith("HTTP/")) {
			...
			throw new IOException(translate("Proxy connection failed. It does not seem to be a valid http-proxy because this response is unexpected: %1%s", error));
		}
		...
		if (!answer.endsWith(" 200")) {
			...
			br.close();
			throw new IOException(translate("Proxy connection failed: %1$s", error));
		}
		...
		while (startTime + TIMEOUT * 1000 > System.currentTimeMillis()) {
			int b = is.read();
			if (b < 0) {
				throw new IOException(translate("Unexpected end of stream while reading proxy preload."));
			}
		...
		throw new IOException(translate("Timeout while reading proxy preload"));
	}
